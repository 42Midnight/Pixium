...
export default function TitleBar({ title, onBack }: TitleBarProps) {
  ...
  return (
    <div className={styles.titlebarDrag} style={{ display: 'flex', alignItems: 'center', padding: '0 16px', WebkitAppRegion: 'drag', backgroundColor: 'rgba(51, 51, 51, 1)', height: '60px' } as React.CSSProperties}>
      {/* Left: back button */}
      <div style={{ WebkitAppRegion: 'no-drag', width: 40, flexShrink: 0 } as React.CSSProperties}>
        {onBack && (
          <button className={styles.winCtrlBtn} onClick={onBack} title="返回">
      ...
      {/* Center: title */}
      <div className={styles.titlebarTitle} style={{ flex: 1, height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', WebkitAppRegion: 'drag' } as React.CSSProperties}>
      ...
      <div style={{ WebkitAppRegion: 'no-drag', display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 } as React.CSSProperties}>
        <button className={`${styles.pinButton} ${alwaysOnTop ? styles.active : ''}`} onClick={handleTogglePin} title={alwaysOnTop ? '取消置顶' : '窗口置顶'}>
        ...
        <button className={styles.winCtrlBtn} onClick={() => { if (location.pathname !== '/favorites') navigate('/favorites', { state: { previousPath: location.pathname, previousState: location.state } }); }} title="喜欢">
        ...
        <button className={styles.winCtrlBtn} onClick={() => { if (location.pathname !== '/settings') navigate('/settings', { state: { previousPath: location.pathname, previousState: location.state } }); }} title="设置">
        ...
        <button className={styles.winCtrlBtn} onClick={handleMinimize} title="最小化">
          <svg width="14" height="14"><line x1="2" y1="7" x2="12" y2="7" stroke="#fff" strokeWidth="1.2"/></svg>
        </button>
        <button className={styles.winCtrlBtn} onClick={handleMaximize} title="最大化">
          {isMaximized
        ...
        <button className={`${styles.winCtrlBtn} ${styles.winCloseBtn}`} onClick={handleClose} title="关闭">
